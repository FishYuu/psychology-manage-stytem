        ...
        <div class="gem-guy-body">
          <div class="filter-container">
            <el-select v-model="selectValue" filterable remote placeholder="输入姓名或手机号搜索" :remote-method="remoteMethod" :loading="remoteLoading" @change="onSelectChange" size="mini">
              <el-option v-for="item in optionsData" :key="item.userId" :label="item.userName" :value="item.userId"> </el-option>
            </el-select>
          </div>
          <div class="select-body">
            ...
            <div class="selected-tree-body">
              ...
              <div class="selected-container" v-show="haveSelectedData">
                <el-tag v-for="tag in selectedListData" :key="tag.userId" closable size="medium" @close="deleteFromSelected(tag)">
                  {{ tag.userName }}
                </el-tag>
              </div>
...
export default {
  ...
  props: {
    ...
    deptId: {
      type: String,
      default() {
        // return JSON.parse(window.sessionStorage.getItem('loginUser')) ? JSON.parse(window.sessionStorage.getItem('loginUser')).deptId : ''
        return config.APP_ROOT_CODE
      }
    },
    userFlag: {
      type: String,
  ...
  methods: {
    ...
    onSelectChange(ret) {
      let data = this.optionsData.find(v => v.userId === ret)
      if (this.selectedListData.findIndex(v => v.userId === data.userId) === -1) {
    ...
    loadNode(node, resolve) {
      let param = {
        deptId: node.level === 0 ? this.deptId : node.data.deptId,
        userFlag: this.userFlag,
        userStatusList: this.userStatusList
      }
      this.$http(Api.queryDept, param).then(ret => {
        let formatDeptList = ret.deptTree.subDeptList
          ? ret.deptTree.subDeptList.map(dept => {
              return {
                gemName: dept.deptName,
                gemType: 'dept',
                ...dept
              }
            })
          : []
        let formatUserList = ret.deptTree.userList
          ? ret.deptTree.userList.map(user => {
              return {
          ...
          : []
        resolve([...formatDeptList, ...formatUserList])
      })
    },
    selectOne(data) {
      if (this.selectedListData.findIndex(v => v.userId === data.userId) === -1) {
    ...
    deleteFromSelected(data) {
      this.selectedListData.splice(
        this.selectedListData.findIndex(v => v.userId === data.userId),
        1
      )
    },
    triggleResult() {
